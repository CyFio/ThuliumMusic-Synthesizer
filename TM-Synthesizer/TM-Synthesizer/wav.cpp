...

{
	...
	if (!write(wave, "fmt ", 4))return false;
	if (!write(wave, &dwFmtSize, sizeof(DWORD)))return false;
	if (!write(wave, &wavfmt.wFormatTag, sizeof(WORD)))return false;
	if (!write(wave, &wavfmt.nChannels, sizeof(WORD)))return false;
	if (!write(wave, &wavfmt.nSamplesPerSec, sizeof(DWORD)))return false;
	if (!write(wave, &wavfmt.nAvgBytesPerSec, sizeof(DWORD)))return false;
	if (!write(wave, &wavfmt.nBlockAlign, sizeof(WORD)))return false;
	if (!write(wave, &wavfmt.wBitsPerSample, sizeof(WORD)))return false;
	if (!wave.write((char*)&wavfmt.cbSize, sizeof(WORD)))return false;
	/*write fact chunk*/
	if (!write(wave, "fact", 4))return false;
	if (!write(wave, &dwFactSize, sizeof(DWORD)))return false;
	if (!write(wave, lpFact, dwFactSize))return false;
	/*write data chunk*/
	if (!write(wave, "data", 4))return false;
	if (!write(wave, &bytecount, sizeof(DWORD)))return false;
	if (!write(wave, wavedata, bytecount))return false;
	...
	:wavheader(new WAVEFORMATEX), wavedata(nullptr), lpFact(nullptr), state(false), dwDataSize(0)
{
	...
	if (wave.fail())return;//open failed
	dwRiffSize = read_wav_riff(wave);
	if (dwRiffSize < 20)return;
	if (!read_wav_fmt(wave, wavheader))return;
	if (wavheader->cbSize != 0)
	{
		lpFact = new char[wavheader->cbSize];
		wave.seekg(2 * sizeof(DWORD), wave._Seekcur);
	...
	if (!read_wav_data(wave, wavedata, &dwDataSize))return;
	state = true;
	return;
}

wavfile::wavfile(WAVEFORMATEX* header, BYTE* wavedata, size_t bytesize)
	:wavheader(new WAVEFORMATEX), wavedata(nullptr), lpFact(nullptr), state(false), dwDataSize(0)
{
}

wavfile::wavfile(BYTE* wavedata, size_t bytecount)
	: wavheader(new WAVEFORMATEX), wavedata(nullptr), lpFact(nullptr), state(false), dwDataSize(0)
{
}
